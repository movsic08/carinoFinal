          ...
          <div class="ps-3">
            <nav aria-label="Breadcrumb Navigation">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Deleted Assessment Table</li>
              </ol>
            </nav>
          </div>
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="example" class="table table-striped table-hover table-bordered text-center">
                ...
                <tbody>
                  @forelse ($deletedAssessmentRecords as $index => $deletedAssessmentRecord)
                  <tr class="align-middle">
                      <td>{{ $index + 1 }}</td>
                      <td>{{ $deletedAssessmentRecord->assessment_client_name }}</td>
                      <td>{{ $deletedAssessmentRecord->date_of_visit }}</td>
                      <td>{{ $deletedAssessmentRecord->method_accepted }}</td>
                      <td>{{ $deletedAssessmentRecord->deleted_at }}</td>
                      <td>
                        <form method="POST" action="{{ route('staff-restore-assessment-record', ['id' => $deletedAssessmentRecord->id]) }}">
                              @csrf
                              <button type="submit"  class="btn btn-xs btn-warning" onclick="return confirm('Are you sure you want to restore this assessment record?')">
                                <i class='bx bx-trash-alt'></i> Restore
                              </button>
                          </form>
                      </td>
                  </tr>
              @empty
                  <tr>
                      <td colspan="6" class="text-center">No soft-deleted assessment records found.</td>
                  </tr>
              @endforelse
