  ...
  <div class="wrapper">
    ...
    <div class="page-wrapper">
      <div class="page-content">
          ...
          <div class="ps-3">
            <nav aria-label="Breadcrumb Navigation">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Deleted Client Table</li>
              </ol>
            </nav>
          </div>
        ...
        <hr />
        <form method="GET" action="{{ route('filter-deleted-clients') }}">
          @csrf
          <div class="card">
            <div class="card-body">
                <div class="row">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="example" class="table table-striped table-hover table-bordered text-center">
                ...
                <tbody>
                    @foreach ($deletedClients as $index => $deletedClient)
                        <tr class="align-middle">
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $deletedClient->first_name . ' ' . ($deletedClient->middle_name ? $deletedClient->middle_name . ' ' : '') . $deletedClient->last_name }}</td>
                            <td>{{ $deletedClient->client_code }}</td>
                            <td>{{ $deletedClient->method_accepted }}</td>
                            <td>{{ $deletedClient->deleted_at }}</td>
                            <td>
                              <a href="{{ route('restore-client', ['encrypted_id' => Crypt::encrypt($deletedClient->id)]) }}" class="btn btn-xs btn-warning" onclick="return confirm('Are you sure you want to restore this client?')">
